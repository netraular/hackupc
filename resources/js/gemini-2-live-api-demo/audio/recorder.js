...
export class AudioRecorder extends EventTarget {
    ...
    async start(onAudioData) {
        ...
        try {
            ...
            // Load and initialize audio processing worklet with debug logs
            try {
                console.debug('Attempting to load audio worklet module...');
                await this.audioContext.audioWorklet.addModule('js/audio/worklets/audio-processor.js');
                console.debug('Audio worklet module loaded successfully.');
                this.processor = new AudioWorkletNode(this.audioContext, 'audio-recorder-worklet');
                console.debug('AudioWorkletNode created successfully.');
            } catch (error) {
